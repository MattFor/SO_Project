...
static void terminate_service_graceful(pid_t& pid, const std::string& name)
{
	...
	for (int i = 0; i < 10; i++)
	{
		if (pid_t w = waitpid(pid, &st, WNOHANG); w == pid)
		{
	...
	if (is_pid_alive(pid))
	{
		...
		if (pid_t w2 = waitpid(pid, &st, 0); w2 == pid)
		{
...
static bool setup_ipc()
{
	...
	mqd_t mqd;
	mqd = mq_open(MQ_REG_NAME, O_CREAT | O_RDWR, IPC_MODE, &attr);
	if (mqd == (mqd_t) - 1)
	{
		perror("mq_open reg");
...
{
	auto send = [](pid_t pid, int sig, const char* name)
	{
		if (pid > 0)
		{
			if (kill(pid, sig) == -1)
