...
int main(const int argc, char** argv)
{
	...
	mqd_t mq_doc = mq_open(MQ_DOCTOR_NAME, O_RDONLY);
	if (mq_doc == (mqd_t) - 1)
	{
	...
	while (!evacuation)
	{
		ssize_t r = mq_receive(mq_doc, buf, MAX_MSG_SIZE, &prio);
		if (r == -1)
		{
			if (errno == EINTR)
			{
		...
		const int treat_ms = 200 + (rng() % 800);
