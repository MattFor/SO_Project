...
int main()
{
    ...
    const mqd_t mq_triage = mq_open(MQ_TRIAGE_NAME, O_RDONLY);
    if (mq_triage == (mqd_t)-1)
    {
        perror("mq_open triage read");
        return 1;
    }
    const mqd_t mq_doctor = mq_open(MQ_DOCTOR_NAME, O_WRONLY);
    ...
    while (true)
    {
        ...
        // Send to doctor queue. Use priority: red->8, yellow->5, green->1
        if (const unsigned int send_prio = color == "RED" ? 8 : color == "YELLOW" ? 5 : 1; mq_send(mq_doctor, buf, sizeof(PatientInfo), send_prio) == -1)
        {
