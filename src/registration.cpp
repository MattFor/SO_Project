...
int main(int argc, char** argv)
{
	...
	// Open message queues
	mqd_t mq_reg = mq_open(MQ_REG_NAME, O_RDONLY);
	if (mq_reg == (mqd_t) - 1)
	{
	...
	mqd_t mq_triage = mq_open(MQ_TRIAGE_NAME, O_WRONLY);
	...
	while (true)
	{
		...
		ssize_t r = mq_receive(mq_reg, buf, MAX_MSG_SIZE, &prio);
		if (r == -1)
		{
			if (errno == EINTR)
				continue;
		...
		if (g_shm->current_inside < g_shm->N_waiting_room)
		{
			...
			{
				char mqname[256];
				snprintf(mqname, sizeof(mqname), "%s%u", PATIENT_CTRL_MQ_PREFIX, (unsigned)p.pid);
				mqd_t mq = mq_open(mqname, O_WRONLY | O_NONBLOCK);
				if (mq != (mqd_t) - 1)
				{
					ControlMessage cm{};
					cm.cmd = CTRL_INSIDE;
